...
def final_classify(texts: list[str]) -> list[dict]:
    ...
    for text, (rule_label, rule_conf, debug), (clf_label, clf_conf) in zip(texts, rule_results, clf_results):
        ...
        if task_analysis.get("is_task"):
            ...
            source = "rules+task_filter"

        elif question_kind in {"main", "followup"}:
            label = "question"
            confidence = max(float(rule_conf), float(clf_conf) if clf_label == "question" else 0.0, 0.94)
            source = "rules+question_filter"

        elif clf_conf >= max(CLASSIFIER_CONFIDENCE + 0.08, 0.86):
            if not debug.get("filler") and not is_progress_text(clean):
                if clf_label == "answer" and is_good_answer(clean):
                    label = "answer"
                    confidence = clf_conf
                    source = "classifier+answer_filter"

        if label == "task" and not task_analysis.get("is_task"):
            label = "other"
            confidence = 0.49
            source = "task_filter_reject"

        if label == "question" and question_kind not in {"main", "followup"}:
            label = "other"
            confidence = 0.49
            source = "question_filter_reject"

        if label == "answer" and not is_good_answer(clean):
            label = "other"
            confidence = 0.49
            source = "answer_filter_reject"
...
def detect_tasks(segments: list[dict], classified: list[dict], sentiments: list[dict]) -> list[dict]:
    ...
    for seg, cls, sent in zip(segments, classified, sentiments):
        ...
        tasks.append({
            ...
            "deadline": deadline,
            "confidence": round(max(float(cls.get("confidence", 0.0)), float(task_analysis.get("score", 0.91))), 4),
            "sentiment": sent,
            "status": "new",
